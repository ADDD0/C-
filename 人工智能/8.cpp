...
class Syncretism {
    ...
    public:
        ...
        string change(string f, Transform t); //用代换q对公式f进行合一代换
...
bool Syncretism::Issyn(string f1, string f2, vector<Transform> &lan) {
    while (!same(f1, f2)) {
        ...
        if (!flag)
            ...
            lan.push_back(t); //将t加入vector lan中
            if(flag) {
...
void Syncretism::show() {
    ...
    cout << " 常量 : 形如 a,b,c,d(a-g) 等 " << endl << " 变量 : 形如 x,y,z,u 等" << endl;
    string f1, f2;
    cout << " 输入 F1:";
    cin >> f1;
    cout << " 输入 F2:";
    ...
    if (Issyn(f1, f2, lan)) {
        if (same(f1, f2)) { //如果 f1,f2 相同则合一为ε
            cout << " 合一 σ = ε " << endl;
            return ;
        }
        cout << " 合一 σ ={ ";
